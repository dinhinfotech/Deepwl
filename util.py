...
def deepwl(graph=None, iters_features=None, n_hop=None):
    ...
    for u in range(N):
        ...
        for iter_id in range(n_iter):
            u_vec_sum = csr_matrix(iters_features[iter_id][0][dict_hops_neighbors[u],:].sum(axis=0))
            
            for v in range(u, N):
                dot_value_check = iters_features[iter_id][0][u].multiply(iters_features[iter_id][0][v]).sum()
                if dot_value_check != 0:                    
                    v_vec_sum = csr_matrix(iters_features[iter_id][0][dict_hops_neighbors[v],:].sum(axis=0))
                    
                    G[u, v] += dot_value_check* u_vec_sum.multiply(v_vec_sum).sum()
                    G[v,u] = G[u,v]
